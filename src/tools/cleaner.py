...
def clean_ats(df: pd.DataFrame, iso_classes: pd.DataFrame) -> pd.DataFrame:
...
def clean_home_care(df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    df['CareType'] = df['CareType'].map(clean_string)
