...
def get_dropouts(data, start_at_ts, citizen_ids, dropout_threshold):
    sum_of_care = np.sum(data[:,:,2:], axis=2)
    zero_ranges = np.array(list(map(zero_runs, sum_of_care)), dtype=object)
...
def get_first_event(alarm_ts: float, dropout_ts: float) -> str:
...
def make_alarm_label(df: pd.DataFrame, start_at_ts: np.ndarray,
    ...
    for citizen_id in citizen_ids:
        ...
        first_event = get_first_event(alarm_ts, dropout_ts)
