...
def predict_survival_funciton(model, X_test, y_test) -> pd.DataFrame:
...
def predict_hazard_function(model, X_test, y_test) -> pd.DataFrame:
    lower, upper = np.percentile(y_test[y_test.dtype.names[1]], [10, 90])
    times = np.arange(lower, upper+1, dtype=int)
    surv_prob = np.row_stack([fn(times) for fn in model.predict_cumulative_hazard_function(X_test)])
    return pd.DataFrame(surv_prob, columns=times)

def predict_td_risk_scores(model, X_test, y_train, y_test, time_bins) -> pd.DataFrame:
...
def paired_ttest_5x2cv(estimator1, estimator2, estimator1_best_fts,
